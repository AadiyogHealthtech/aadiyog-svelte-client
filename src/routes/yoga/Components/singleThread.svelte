<script lang="ts">
	...
	export function getScaledIdealKeypoint(ctx, idx, idealKps, userKps, hipPoint, scalingFactor) {
		...
		// 4) Scale the ideal vector
		const scaledRaw = [idealDir.raw[0] * scale, idealDir.raw[1] * scale];

		// 5) Reconstruct corrected ideal in normalized space
		const correctedNorm = [hipPoint[0] + scaledRaw[0], hipPoint[1] + scaledRaw[1]];

		// 6) Convert to pixel coords
		const correctedPix = [correctedNorm[0] * width, correctedNorm[1] * height];
	...
	export function checkPoseSuccess(idealKeypoints, normalizedKeypoints, thresholds) {
		...
		for (const idx of indices) {
			const { dtwDistance } = calculateDtwScore([idealKeypoints[idx]], [normalizedKeypoints[idx]]);
	...
	export function checkBendback(
		...
		const { dtwDistance: dtwRightWrist } = calculateDtwScore(
			[idealKeypoints[16]],
			[normalizedKeypoints[16]]
		);
		const { dtwDistance: dtwLeftWrist } = calculateDtwScore(
			[idealKeypoints[15]],
			[normalizedKeypoints[15]]
		);
		const { dtwDistance: dtwLeftShoulder } = calculateDtwScore(
			[idealKeypoints[11]],
			[normalizedKeypoints[11]]
		);
		const { dtwDistance: dtwRightShoulder } = calculateDtwScore(
			[idealKeypoints[12]],
			[normalizedKeypoints[12]]
		);
		const { dtwDistance: dtwLeftElbow } = calculateDtwScore(
			[idealKeypoints[13]],
			[normalizedKeypoints[13]]
		);
		const { dtwDistance: dtwRightElbow } = calculateDtwScore(
			[idealKeypoints[14]],
			[normalizedKeypoints[14]]
		);
		const { dtwDistance: dtwLeftKnees } = calculateDtwScore(
			[idealKeypoints[25]],
			[normalizedKeypoints[25]]
		);
		const { dtwDistance: dtwRightKnees } = calculateDtwScore(
			[idealKeypoints[26]],
			[normalizedKeypoints[26]]
		);
		const { dtwDistance: dtwLeftAnkle } = calculateDtwScore(
			[idealKeypoints[25]],
			[normalizedKeypoints[25]]
		);
		const { dtwDistance: dtwRightAnkle } = calculateDtwScore(
			[idealKeypoints[26]],
			[normalizedKeypoints[26]]
		);

		const LeftWristTh =
			thresholds[15] * calculateEuclideanDistance(idealKeypoints[15], idealKeypoints[23]);
		const RightWristTh =
			thresholds[16] * calculateEuclideanDistance(idealKeypoints[16], idealKeypoints[23]);
		const LeftShoulderTh =
			thresholds[11] * calculateEuclideanDistance(idealKeypoints[11], idealKeypoints[23]);
		const RightShoulderTh =
			thresholds[12] * calculateEuclideanDistance(idealKeypoints[12], idealKeypoints[23]);
		const LeftElbowTh =
			thresholds[13] * calculateEuclideanDistance(idealKeypoints[13], idealKeypoints[23]);
		const RightElbowTh =
			thresholds[14] * calculateEuclideanDistance(idealKeypoints[14], idealKeypoints[23]);
		const LeftKneeTh =
			thresholds[25] * calculateEuclideanDistance(idealKeypoints[25], idealKeypoints[23]);
		const RightKneeTh =
			thresholds[26] * calculateEuclideanDistance(idealKeypoints[26], idealKeypoints[23]);
		const LeftAnkleTh =
			thresholds[27] * calculateEuclideanDistance(idealKeypoints[27], idealKeypoints[23]);
		const RightAnkleTh =
			thresholds[28] * calculateEuclideanDistance(idealKeypoints[28], idealKeypoints[23]);
		const modified = {
			11: LeftShoulderTh,
			12: RightShoulderTh,
			13: LeftElbowTh,
		...
		const thresholds_new = thresholds.map((oldTh, i) =>
			...
			modified.hasOwnProperty(i) ? modified[i] : oldTh
		);
		...
		process(currentTime) {
			...
			if (nk) {
				...
				if (this._pointQueue.length > 0) {
					...
					const ratio = this._originalLength > 0 ? done / this._originalLength : 1;
					pathGood = ratio >= 0.7;
		...
		getIdealKeypoints(phase) {
			...
			if (segment.phase === phase) {
				...
				console.log(
					...
					this.yoga.getIdealKeypoints(representativeFrame, representativeFrame + 1)[0] || []
				);
	...
	async function initPoseLandmarker() {
		...
		if (storedLandmarker) {
			...
			try {
				...
				poseLandmarker = await vision.PoseLandmarker.createFromOptions(wasmFileset, {
					baseOptions: {
						modelAssetPath:
				...
				if (results?.landmarks?.length > 0) {
					for (const landmarks of results.landmarks) {
						...
						if (userInPosition && controller && controllerInitialized) {
							...
							currentTime += 1 / 60;
							const [currentPhase, exerciseName, repCount, targetReps] =
	...
	// Component Logic
	$: currentWorkout =
		$allWorkouts.find((workout) => workout.title === yogName) || $allWorkouts[0] || null;
...
<div class="h-surface flex flex-col overflow-hidden relative w-full">
	...
	{#if showProgressBar}
		<div class="fixed top-0 inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
			<div class="bg-white rounded-lg p-6 w-80 shadow-xl">
				...
				<div class="text-center mb-2 font-medium text-gray-700">Loading Exercises...</div>
	...
	>
		...
		<canvas id="output_canvas" class="pointer-events-none absolute top-0 left-0 z-0 w-full h-full"
		></canvas>
    ...
    {/if} -->
				...
				<div class="video-intro-overlay">
					<div class="video">
						<iframe
							width="100%"
							height="100%"
							src="https://www.youtube.com/embed/OAD3Ju04xHs?autoplay=1&mute=1&loop=1&controls=0&color=white&modestbranding=0&rel=0&playsinline=1&enablejsapi=1&playlist=OAD3Ju04xHs"
							title="Yoga Pose Intro"
							frameborder="0"
		...
		{#if !userInPosition && !dimensions.startsWith('Camera error') && !dimensions.startsWith('Pose landmarker error')}
			<div
				class="absolute left-1/2 transform -translate-x-1/2 z-20 flex justify-between items-center w-full px-4 z-50"
				style="bottom: 5%; 
           max-width: {isInitialized ? targetBox.width : '90%'};"
			>
				...
				<button
					on:click={handlePlay}
					class="bg-white p-4 rounded-full relative shadow-lg focus:outline-none hover:bg-gray-100"
				>
		...
		{#if userInPosition}
			<div class="user-in-position-container">
				<!-- <div class="score-reps-container">
					...
					<div
						class="flex items-center border-2 border-orange-400 px-2 py-1 rounded-lg bg-white bg-opacity-80"
					>
						<div class="flex flex-col mr-8">
		...
		<div class="fixed inset-0 flex items-center justify-center z-50">
			<div class="bg-white p-6 rounded-lg shadow-lg">
				...
				<div class="flex justify-end space-x-4">
					<button on:click={cancelStop} class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400"
						>Cancel</button
					>
					<button
	...
	{#if showInstructionalModal}
		<div class="fixed inset-0 flex items-center justify-center z-50 p-2 mb-8">
			<div
				class="bg-white p-6 shadow-lg w-[96vw] h-[92vh] overflow-hidden instructional-modal flex flex-col"
			>
				<div class="flex-grow overflow-y-auto hide-scrollbar">
					<video
						src={currentWorkout?.videoUrl ||
							'https://example.com/anuvittasana-instructional-video.mp4'}
						controls
						autoplay
				...
				<div
					class="fixed bottom-4 left-0 right-0 flex justify-between items-center px-4 py-2 bg-gray-500"
				>
					<button
						on:click={handleVideoButtonClick}
						class="h-16 w-16 overflow-hidden focus:outline-none bg-white shadow-xl hover:bg-gray-100 rounded-full"
					>
						<img src={nexticon} alt="Next button" class="h-full w-full object-cover rounded-full" />
					</button>

					<button
						on:click={closeInstructionalModal}
						class="bg-white p-4 rounded-full shadow-xl focus:outline-none hover:bg-gray-100"
					>
						<svg class="w-10 h-10 text-black" viewBox="0 0 24 24">
							<path d="M10 8l6 4-6 4V8z" fill="currentColor" />
						</svg>
...
<style>
	...
	/* Ensure popup stays above button bar */
	.video-intro-wrapper {
		position: fixed;
		top: 0;
		left: 0;
	...
	/* Card with fixed aspect and spacing from bottom buttons */
	.video-intro-overlay {
		width: 90vw;
		max-width: 342px;
		height: auto;
		...
		/* Exact height with responsive offset from button area */
		height: calc(687 / 812 * 100vh - 102px); /* 812 is base figma height */
		margin-bottom: calc(102 / 812 * 100vh); /* space for buttons */
	}
