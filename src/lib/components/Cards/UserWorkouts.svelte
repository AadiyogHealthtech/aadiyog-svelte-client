<script lang="ts">
  ...
  function goToUserProfile() {
    const userId = post.user?.id; // Extract user ID
    console.log(post)
    goto(`/profile/${userId}`); // Navigate to the profile page using the user ID
  }
</script>
...
{/if}
    ...
    <h3 class="mt-4 text-gray-600">{liked} {post.likes === 1 ? 'Like' : 'Likes'}</h3>
  </div>
