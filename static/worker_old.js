...
function checkPoseSuccess(idealKeypoints, normalizedKeypoints, thresholds) {
	...
	const { dtwDistance: dtwHand } = calculateDtwScore(
		idealKeypoints.slice(13, 21),
		normalizedKeypoints.slice(15, 21)
	);
	const { dtwDistance: dtwShoulder } = calculateDtwScore(
		[idealKeypoints[11], idealKeypoints[12]],
		[normalizedKeypoints[11], normalizedKeypoints[12]]
	);
...
function checkBendback(ctx, idealKeypoints, normalizedKeypoints, currentTime, thresholds) {
	...
	const { dtwDistance: dtwTorso } = calculateDtwScore(
		[idealKeypoints[15]],
...
function norm(p) {
	return function (a, b) {
		const diff = Array.isArray(a) ? a.map((val, i) => val - b[i]) : [a - b];
		return Math.pow(
			diff.reduce((sum, val) => sum + Math.pow(Math.abs(val), p), 0),
			1 / p
		);
	};
}
...
function prepInputs(x, y, dist) {
	...
	if (
		xArr.length > 1 &&
		yArr.length > 1 &&
		Array.isArray(xArr[0]) &&
		Array.isArray(yArr[0]) &&
		xArr[0].length !== yArr[0].length
	) {
...
function dtwCore(x, y, window, dist) {
	...
	for (const [i, j] of fullWindow.map(([i, j]) => [i + 1, j + 1])) {
		...
		D.set(
			`${i},${j}`,
			options.reduce((min, curr) => (curr[0] < min[0] ? curr : min))
		);
	}
			...
			for (const path of transitionAnalyzer.transitionPaths) {
				if (
					...
					if (
						...
						const withinPath = transitionAnalyzer.analyzeTransition(
...
class Controller {
	...
	_initializeHandlers() {
		...
		this.segments.forEach((segment, i) => {
			...
			if (phaseType === 'starting' || phaseType === 'ending') {
				...
				const keypointsFrame = idealKeypoints[middleIdx] || [];
				console.log(
					`[Controller] Segment ${uniqueKey} - Raw ideal keypoints sample:`,
					keypointsFrame.slice(0, 5)
				);
				if (keypointsFrame.length >= 33) {
					const normalizedIdealKeypoints = normalizeKeypoints(keypointsFrame);
					console.log(
						`[Controller] Segment ${uniqueKey} - Normalized ideal keypoints sample:`,
						normalizedIdealKeypoints?.slice(0, 5)
					);
					if (normalizedIdealKeypoints) {
						const calculatedFacing = detectFacing(normalizedIdealKeypoints);
						console.log(
							`[Controller] Segment ${uniqueKey} - Calculated facing: ${calculatedFacing}, Precomputed facing: ${segment.facing}`
						);
						startFacing = calculatedFacing !== 'random' ? calculatedFacing : segment.facing;
					}
				} else {
					console.warn(
						`[Controller] Segment ${uniqueKey} - Insufficient keypoints for facing detection, using precomputed: ${startFacing}`
					);
				}
			}
			...
			if (phaseType === 'starting') handlers[uniqueKey] = new StartPhase(this, startFacing);
				...
				handlers[uniqueKey] = new HoldingPhase(this, segment.thresholds);
	...
	processExercise(currentTime) {
		...
		console.log(
			`[Controller] Processing segment: ${currentSegment.phase}, Completed: ${completed}`
		);
		...
		if (completed) {
			...
			if (currentSegment.type === 'ending') {
				...
				if (
					this.count >= this.targetReps &&
					this.currentExerciseIdx < this.exerciseNames.length - 1
				) {
