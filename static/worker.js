		...
		if (this.controller.normalizedKeypoints) {
			...
			if (success) {
				...
				if (this.completedHold && dtwLeftWrist > 0) {
					// this is just for testing in real life we have to incerease it to exitthresholdwrist
...
function checkBendback(ctx, idealKeypoints, normalizedKeypoints, hipPoint, thresholds) {
	...
	const shoulTh =
		thresholds[2] * calculateEuclideanDistance(idealKeypoints[11], idealKeypoints[23]);

	const success = checkPoseSuccess(idealKeypoints, normalizedKeypoints, [
		thresholds[0],
		handTh,
		shoulTh
	]);
...
class YogaDataExtractor {
	constructor(jsonData) {
		...
		console.log(
			'[YogaDataExtractor] Initialized with frames:',
			this.keypointsData.length,
			'segments:',
			this.segmentsData.length
		);
	}
...
class TransitionAnalyzer {
	...
	_toPixelCoords(point, width, height) {
		...
		return [((x + 1) * safeWidth) / 2, ((y + 1) * safeHeight) / 2];
	}
